...
func QueryTxsRequestHandlerFn(clientCtx client.Context) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		limitFromURL := r.URL.Query().Get("limit")
		limitFromForm := r.FormValue("limit")
		eventsFromForm := r.FormValue("events")
		logrus.Println("limitFromURL: ", limitFromURL)
		logrus.Println("limitFromForm: ", limitFromForm)
		logrus.Println("EventsFromForm: ", eventsFromForm)
		var (
			events      []string
			txs         []sdk.TxResponse
			page, limit int
		...
		logrus.Println("events ParseHTTPArgs: ", events)
		logrus.Println("page ParseHTTPArgs: ", page)
		logrus.Println("limit ParseHTTPArgs: ", limit)
